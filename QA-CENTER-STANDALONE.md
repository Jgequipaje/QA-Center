...
- Dark / light toggle
- Design tokens system (same as PurrScope — `tokens.dark` / `tokens.light`)
...
All routes are thin — business logic lives in `server/routes/`.
...
Since this is local-only, the easiest way to share it is just git:

```bash
# Share the whole qa-center/ folder in your repo
# Each person clones and runs:
npm install && npm run build && node bin/cli.js
```

`qa-issues.json` can be committed to share issue state across the team,
or added to `.gitignore` if you want it to stay personal.
