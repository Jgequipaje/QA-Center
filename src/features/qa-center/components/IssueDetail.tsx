      ...
      <div style={{ padding: "14px 16px", overflowY: "auto", flex: 1 }}>
        ...
        >
          {editing ? (
            <input
              ...
              style={{ ...inputStyle, fontSize: 13, fontWeight: 700, flex: 1, marginRight: 8 }}
              placeholder="Issue title"
            />
          ) : (
            <span style={{ fontSize: 13, fontWeight: 700, color: t.text, flex: 1, marginRight: 8 }}>
              {issue.title}
            </span>
          )}
          <button
            ...
            style={{
              ...
              color: running || saving ? t.textFaint : t.textMuted,
              fontSize: 16,
              padding: 0,
            }}
            title={running || saving ? "Operation in progress..." : undefined}
          >
            ✕
          </button>
        </div>
        ...
        {!editing && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
            ...
            {issue.severity && origin !== "note" && (
              <Badge
                label={origin === "feature" ? `Priority: ${issue.severity}` : issue.severity}
                color={t.warnText}
                bg={t.warnBg}
              />
            )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div>
              ...
              <textarea
                ...
                rows={origin === "note" ? 8 : 3}
                style={{ ...inputStyle, resize: "vertical" }}
                placeholder={origin === "note" ? "Write your note..." : "What is the issue?"}
          ...
          <>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span
                    style={{
                      fontSize: 11,
                      fontWeight: 700,
                      color: RESULT_COLOR[autoResult] ?? t.textFaint,
                    }}
                  >
                    {autoResult === "not_run"
                      ? "Not run yet"
                ...
                {running && (
                  <div style={{ fontSize: 11, color: t.infoText, marginTop: 4 }}>
                    ⏳ Test running — keep this panel open...
                  </div>
                )}
              </div>
            )}
            ...
            {(issue.expected || issue.actual) && (
              <div style={{ marginTop: 12 }}>
                ...
                {issue.expected && (
                  <div style={{ color: t.passText, fontSize: 12, marginBottom: 4 }}>
                    Expected: {issue.expected}
                  </div>
                )}
                {issue.actual && (
                  <div style={{ color: t.failText, fontSize: 12 }}>Actual: {issue.actual}</div>
                )}
              </div>
            )}
            {issue.notes && (
          ...
          <>
            ...
            {available.map((to) => {
              ...
              return (
                <button
                  key={to}
                  onClick={() => !blocked && updateIssueStatus(baseUrl, issue.id, to)}
                  disabled={blocked}
                  title={blockReason(to) ?? undefined}
                  data-testid={`detail-status-${to}`}
                  style={{
                    padding: "5px 12px",
                    fontSize: 12,
                    fontWeight: 600,
